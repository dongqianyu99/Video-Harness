...
def test_mock_cli_runs_full_multiview_media_pipeline_without_debug(tmp_path: Path) -> None:
    ...
    annotation = annotated["guidance_units"][0]["annotation"]
    assert annotation["schema_version"] == EVIDENCE_SCHEMA_VERSION
    assert annotation["status"] == "mock"
    assert annotation["record"]["review_status"] == "needs_review"
    assert annotation["record"]["causal_validation"]["status"] == "retry"
    assert set(annotation["provenance"]) == {"call1", "call2"}
    assert annotation["provenance"]["call2"]["attempts"] == 3
    assert annotation["provenance"]["call2"]["selected_attempt"] == 3
