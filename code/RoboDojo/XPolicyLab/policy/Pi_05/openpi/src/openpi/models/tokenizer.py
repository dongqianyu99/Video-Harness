...
class PaligemmaTokenizer:
    ...
    def tokenize_text(self, text: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        if token_count > self._max_len:
            raise ValueError(
                f"Guide text token length {token_count} exceeds max_len={self._max_len}"
            )

        padding_count = self._max_len - token_count
        padded_tokens = tokens + [0] * padding_count
        mask = [True] * token_count + [False] * padding_count

        return (
            np.asarray(padded_tokens, dtype=np.int32),
